...
export const IDE_TOOL_PROFILES: IdeToolProfile[] = [
  ...
  },
  {
    ...
    fallbackHint: 'Codex loads provider credentials from an environment variable referenced by env_key. Export CLAW_ROUTER_API_KEY before launching Codex, then point model_providers.clawrouter.base_url at the OpenAI endpoint.',
    stepsKey: 'docs.tools.codexSteps',
    fallbackSteps: '1. Export CLAW_ROUTER_API_KEY with your gateway API key\n2. Update ~/.codex/config.toml with the provider block below\n3. Set model_provider to clawrouter\n4. Restart Codex and choose a supported model ID',
  ...
  },
  {
    ...
    fallbackHint: 'OpenCode resolves apiKey from the CLAW_ROUTER_API_KEY environment variable when you use {env:CLAW_ROUTER_API_KEY} in the config file.',
    stepsKey: 'docs.tools.opencodeSteps',
    fallbackSteps: '1. Export CLAW_ROUTER_API_KEY in your shell\n2. Save the provider block to ~/.config/opencode/opencode.json\n3. Ensure @ai-sdk/openai-compatible is available to OpenCode\n4. Launch opencode and select the clawrouter provider',
  ...
  },
  {
    ...
    fallbackHint: 'OpenClaw reads provider credentials from CLAW_ROUTER_API_KEY when the config uses ${CLAW_ROUTER_API_KEY}. Export the variable before starting OpenClaw.',
...
export function buildIdeToolSnippets(input: IdeToolSnippetInput): IdeToolSnippetMap {
  ...
  return {
    ...
    birdcoder: [
      ...
      '      "customModels": [',
      '        { "id": "gpt-4o-mini", "label": "Claw Router Codex" }',
      '      ]',
      '    },',
      '    "claude-code": {',
      '      "selectedModelId": "claude-3-5-sonnet",',
      '      "customModels": [',
      '        { "id": "claude-3-5-sonnet", "label": "Claw Router Claude" }',
      '      ]',
      '    },',
      '    "gemini": {',
      '      "selectedModelId": "gemini-2.0-flash",',
      '      "customModels": [',
      '        { "id": "gemini-2.0-flash", "label": "Claw Router Gemini" }',
      '      ]',
      '    },',
      '    "opencode": {',
      '      "selectedModelId": "gpt-4o-mini",',
      '      "customModels": [',
      '        { "id": "gpt-4o-mini", "label": "Claw Router OpenCode" }',
      '      ]',
      '    }',
      '  }',
      '}',
