...
Read `sdkwork.app.config.json` only when the task touches Documents application behavior, runtime config, SDK wiring, release metadata, app-owned capabilities, packaging, or deployment. For unrelated documentation or tooling work, do not expand into the full app manifest unless evidence requires it.
...
- `sdkwork.app.config.json`: Documents application identity, runtime, release, and capability metadata.
...
- `apis/`: Documents-owned API contract sources and materialized OpenAPI inputs.
...
Read `../sdkwork-specs/CODE_STYLE_SPEC.md` and `../sdkwork-specs/NAMING_SPEC.md` before code changes. Keep edits inside the owning module, package, crate, app root, or standard tool. Generated SDK transport output is changed only through source contracts, generator inputs, or approved composed facades. Use `sdkwork-utils-rust` for shared validation and identifier helpers instead of duplicating utility logic locally.
